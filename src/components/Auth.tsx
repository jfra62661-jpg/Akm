...
interface AuthProps {
  onLogin: (phone: string, pin: string, isRegister: boolean, email?: string) => void;
  error?: string;
  isAdminLogin?: boolean;
}

export default function Auth({ onLogin, error, isAdminLogin }: AuthProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onLogin(phone, pin, isRegister, email);
  };
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-black/5 p-8">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold mb-2">
            {isAdminLogin ? 'دخول الإدارة' : isRegister ? 'إنشاء حساب جديد' : 'تسجيل الدخول'}
          </h2>
          <p className="text-gray-500 text-sm">
            {isAdminLogin ? 'أدخل بيانات المسؤول للمتابعة' : isRegister ? 'أدخل بياناتك للبدء في الربح' : 'مرحباً بك مجدداً في وسيط آسيا'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1 mr-1">
              {isAdminLogin ? 'رقم الهاتف أو البريد' : 'رقم الهاتف'}
            </label>
            <div className="relative">
              ...
              <input
                type="text"
                required
                placeholder={isAdminLogin ? "07XXXXXXXXX" : "077XXXXXXXX"}
                value={phone}
